...
def avm(pred):
    ...
    guess = {x: ast.Num(random.randint(-10000, 10000)) for x in vari}
    ...
    while True:
        ...
        if not vari:
            return guess
        if guess_depth > 10000:
            return None
        for guess_vari in vari:
            # exploratory move
            guess[guess_vari] = ast.Num(guess[guess_vari].n + 1)
            guess_left = astor.to_source(sub_vari(left, guess)).strip()
            guess_right = astor.to_source(sub_vari(right, guess)).strip()
            f_inc = b_dist(ops, guess_left, guess_right)

            guess[guess_vari] = ast.Num(guess[guess_vari].n - 1)
            guess_left = astor.to_source(sub_vari(left, guess)).strip()
            guess_right = astor.to_source(sub_vari(right, guess)).strip()
            f_dec = b_dist(ops, guess_left, guess_right)

            direction = 1 if f_inc < f_dec else -1
            amp = 1
            prev_f = f_inc if direction == 1 else f_dec

            # pattern move
            while True:
                guess[guess_vari] = ast.Num(guess[guess_vari].n +
                                            direction * amp)
                guess_left = astor.to_source(sub_vari(left, guess)).strip()
                guess_right = astor.to_source(sub_vari(right, guess)).strip()
                f = b_dist(ops, guess_left, guess_right)
                if is_satisfied(ops, f):
                    return guess
                if f < prev_f:
                    amp *= 2
                    prev_f = f
...
def e_stmt(stmt, vari):
    ...
    if type(stmt) == list:
        ...
        if type(stmt.test.left) == ast.Num and type(
            ...
            guess_true = avm(stmt.test)
            guess_false = avm(neg(stmt.test))
